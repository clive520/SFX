...
- 改善音效卡片拖曳排序互動。
- 新增卡片右上角拖曳把手，讓可拖曳狀態更明顯。
- 拖曳時讓卡片浮起跟隨游標，並以虛線放置框顯示放下位置。
- 保留 localStorage 排序記憶；目前 GitHub Pages 純靜態網站只能記住同一瀏覽器的排序，全站共享排序需另接後端資料庫。

- 調整音效區版面比例，長音效約佔三分之一寬度，短音效約佔三分之二寬度。
- 縮小長音效與短音效卡片寬度、高度、間距與按鈕尺寸，讓同一畫面可顯示更多音效卡片。
